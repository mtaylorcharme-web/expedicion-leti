  ...
  function game(m) {
    ...
    const step = () => {
      ...
      w.innerHTML = `<div class="mhead"><button class="close" id="quit" aria-label="Salir">✕</button><div class="pbar" id="tb"><b style="width:100%;background:linear-gradient(90deg,var(--gold),var(--coral))"></b></div><span class="small muted" style="min-width:60px;text-align:right">${i + 1}/${items.length} · ⭐${score}</span></div>
        <div class="lianas"><div class="sky"></div><div class="rama"></div>${items.map((_, k) => `<div class="lstick ${k < i ? "pasada" : k === i ? "aqui" : ""}" style="left:${8 + k * (84 / (items.length - 1))}%"><span class="hoja"></span></div>`).join("")}<div class="suelo"></div><div class="jumper" style="left:${8 + i * (84 / (items.length - 1))}%">${monkey("chupaya", "swing", 64)}</div></div>
