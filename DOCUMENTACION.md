...
- **La selva**: se dibuja en planos. Troncos lejanos delgados y pálidos, troncos medios, y dos troncos cercanos gruesos en los bordes; encima, ramas horizontales a alturas fijas (`ramasY`, en unidades del SVG) con sus racimos de hojas y lianas sueltas colgando. Esas ramas no son decoración: **son de donde cuelgan los Ayas**. La función `anclar()` convierte `ramasY` a píxeles con la altura real del mapa y estira la cuerda de cada Aya (`--cuerda`) hasta la rama que tiene justo encima, así que ninguno cuelga del aire. Se recalcula al cambiar el tamaño de la ventana y cuando Ovaya viaja.
